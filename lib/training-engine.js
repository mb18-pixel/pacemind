/**
 * PaceMind Training Engine – sportwissenschaftliche Logik (deterministisch)
 * -----------------------------------------------------------------------
 * Fokus:
...
export function estimateVDOT(profile = {}) {
  ...
  const distKm = Number.isFinite(refDistKm) && refDistKm > 0 ? refDistKm : profile.ref_10k_zeit ? 10 : 5;

  if (refTimeSec) {
    // Wenn exakt 5K: zusätzlich Tabellen-Fallback (robust)
  ...
  // Fallback nach Fitnesslevel
  const lvl = normalizeFitnesslevel(profile.fitnesslevel);
  if (lvl === "einsteiger") return 30;
  if (lvl === "fortgeschritten") return 55;
  return 42;
}
...
export function computeAthleteLeistungsprofil(profile = {}) {
  ...
  const paces = computeTrainingPacesFromVDOT(vdot);
  const hf = maxHF ? computeHeartRateZones(maxHF) : null;
  return { maxHF, vdot, paces, hf };
}
...
function slotDurationMinutes(slot) {
  const a = parseHmToMinutes(slot?.uhrzeit_start);
  const b = parseHmToMinutes(slot?.uhrzeit_ende);
...
function dateIsoToWeekdayIndex(dateIso) {
  ...
  // JS: So=0 ... Sa=6 -> wir brauchen Mo=0 ... So=6
  const js = d.getDay();
  return js === 0 ? 6 : js - 1;
}
...
function buildLongRunUnit({ distKm, paces, hf, slotDurationMinutes = null }) {
  ...
  let durationMin = Math.round((distKm * longSec) / 60 + 10); // +10' Warm-up/Cool-down
  ...
  if (slotDurationMinutes && durationMin > slotDurationMinutes) {
    ...
    durationMin = Math.round((adjustedDistKm * longSec) / 60 + 10);
  ...
  return {
    ...
    hauptteil: `${round1(distKm)} km sehr locker @ ${formatPaceFromSeconds(longSec)}/km | ${hfZone.min}-${hfZone.max} bpm (${hfZone.label})`,
    ...
    beschreibung: `${round1(distKm)}km @ ${formatPaceFromSeconds(longSec)}/km | HF ${hfZone.min}-${hfZone.max} | ${durationMin} Min`
  };
}
...
function buildEasyUnit({ distKm, paces, hf, slotDurationMinutes = null }) {
  ...
  if (slotDurationMinutes && durationMin > slotDurationMinutes) {
    ...
    adjustedDistKm = Math.min(distKm, maxKm);
    durationMin = Math.round((adjustedDistKm * easy) / 60);
    distanceAdjusted = true;
  }
  ...
  return {
    ...
    hauptteil: `${round1(distKm)} km locker @ ${formatPaceFromSeconds(easy)}/km | ${hfZone.min}-${hfZone.max} bpm (${hfZone.label})`,
    ...
    // BESCHREIBUNG (für Kalender-Karte)
    beschreibung: `${round1(distKm)}km @ ${formatPaceFromSeconds(easy)}/km | HF ${hfZone.min}-${hfZone.max} | ${durationMin} Min`
  };
}
...
function buildRegenUnit({ distKm, paces, hf, slotDurationMinutes = null }) {
  ...
  if (slotDurationMinutes && durationMin > slotDurationMinutes) {
    ...
    adjustedDistKm = Math.min(distKm, maxKm);
    durationMin = Math.round((adjustedDistKm * regenSec) / 60);
    distanceAdjusted = true;
  }
  ...
  return {
    ...
    hauptteil: `${round1(distKm)} km Regeneration @ ${formatPaceFromSeconds(regenSec)}/km | ${hfZone.min}-${hfZone.max} bpm (${hfZone.label})`,
    ...
    // BESCHREIBUNG (für Kalender-Karte)
    beschreibung: `${round1(distKm)}km @ ${formatPaceFromSeconds(regenSec)}/km | HF ${hfZone.min}-${hfZone.max} | ${durationMin} Min`
  };
}
...
function buildTempoUnit({ phase, weekVolKm, paces, hf, slotDurationMinutes = null }) {
  ...
  if (slotDurationMinutes && durationMin > slotDurationMinutes) {
    ...
    durationMin = Math.round((2 * easySec + adjTempoKm * tempoSec + 2 * easySec) / 60);
    distanceAdjusted = true;
  }
...
export function buildTrainingUnit({
  ...
  if (trainingstyp === "langlauf") {
    ...
    return buildLongRunUnit({ distKm: dist, paces, hf });
  }
  if (trainingstyp === "tempo") {
    return buildTempoUnit({ phase: deload ? "basis" : phase, weekVolKm, paces, hf });
  }
  if (trainingstyp === "intervall") {
    // Keine Intervalle in Deload gemäß Vorgabe
    if (deload) return null;
    return buildIntervallUnit({ phase, weekVolKm, vdot, paces, hf });
  }
  if (trainingstyp === "regeneration") {
    const dist = 5; // 4–6 km fix -> Mitte
    return buildRegenUnit({ distKm: dist, paces, hf });
  }
  // default: locker
  // Distanz wird später durch Restvolumen gesetzt -> placeholder 0
  return buildEasyUnit({ distKm: 6, paces, hf });
}
...
export function generateMacroSkeleton(profile = {}) {
  ...
  const maxCap = getMaxVolumeCapKm(fitnesslevel);
  ...
  for (let w = 1; w <= totalWeeks; w++) {
    ...
    if (w === 1) {
      ...
      weekVol = prevWeekVol * 1.08; // +8% Aufbau
    }
 ...
 * chronicLoad = Durchschnitt Wochenvolumen letzte 4 Wochen
 */
export function berechneACWR(runs, currentDate = new Date()) {
  ...
  const acuteRuns = runs.filter(r => {
    ...
    return runDate >= sevenDaysAgo && runDate <= now;
  });
  const acuteLoad = acuteRuns.reduce((sum, r) => sum + (Number(r.distanz_km) || 0), 0);

  // Chronic Load: letzte 4 Wochen (Durchschnitt pro Woche)
  const chronicRuns = runs.filter(r => {
    const runDate = new Date(r.created_at);
    return runDate >= twentyEightDaysAgo && runDate <= now;
  });
  const chronicTotal = chronicRuns.reduce((sum, r) => sum + (Number(r.distanz_km) || 0), 0);
